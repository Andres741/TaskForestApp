...
//        val (taskEntities, subTaskEntities) = taskEntitiesList.unzip() // O(n) performance
    ...
    //suspend fun changeTaskType_Helper()
    ...
    @Query(DELETE_SUB)
    protected abstract suspend fun deleteSubTask(subTask: String)

    @Query(DELETE_SUPER)
    protected abstract suspend fun deleteSuperTask(superTask: String)
}
